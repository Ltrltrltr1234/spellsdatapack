execute as @e[tag=projectile4] at @s run tp @s ^ ^ ^-0.8
execute as @e[tag=projectile4] at @s positioned ~ ~-1.5 ~ unless entity @e[distance=..1,tag=swordproj] run summon armor_stand ~ ~ ~ {Pose:{RightArm:[270f,0f,0f]},DisabledSlots:4144959,Invisible:1b,Tags:["swordproj"],NoGravity:1b,Silent:1b,Invulnerable:1b,CustomNameVisible:0b,NoAI:1b,HandItems:[{id:"minecraft:golden_sword",Count:1b,tag:{Unbreakable:1b,Enchantments:[{}]}},{}],ActiveEffects:[{Id:14b,Amplifier:0b,Duration:99999999}]}
...
execute as @e[tag=projectile4,team=team1,tag=projectiledead] at @s run effect give @e[distance=..3,team=!team1] instant_damage 1 0 true
execute as @e[tag=projectile4,team=team2,tag=projectiledead] at @s run effect give @e[distance=..3,team=!team2] instant_damage 1 0 true
