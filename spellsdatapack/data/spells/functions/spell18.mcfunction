...
execute as @e[scores={rightclick=1..}] at @s run playsound entity.llama.spit master @a ~ ~ ~ 1 1 0
execute as @e[scores={rightclick=1..}] run scoreboard players remove @s mana 50
execute as @e[scores={rightclick=1..}] run tellraw @s {"text":"You used Frost Spear! (-50 Mana)","color":"aqua","bold":true}